...
phyto_raw <- read.csv("phyto.smallzoo.85-20_HAL.csv", sep = ",")  # loading the raw phytoplankton data
zoo_raw <- read.csv("largezoo.85-20_HAL.csv")  # loading the raw zooplankton data
...
lake <- "HAL"  # Defining the name of the lake. This will be used when saving data in external files, to identify them later on
...
rfProfile_knn 
rfProfile_bag 
rfProfile_median # PO4_P and cyclopoida seem to be very important in all 3 cases
...
defaultSummary(data = data.frame(obs = test$r_cyanobac, pred = test_bag))  # The model with the bagging imputation has the best results
...
rfProfile_knn 
rfProfile_bag 
rfProfile_median  # r_cyanobac, PO4, microzoo and cyclopoida seems to be very important predictors
...
# The random forest with median imputation tends to have the best results
...
print(impPlots <- ggarrange(knnPlot, bagPlot,medianPlot,
                            ncol = 1, nrow = 3, align = "hv"))
# r_cyanobac and cyclopoida seems to always be important
