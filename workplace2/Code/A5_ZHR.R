...
# 1) Getting the ZHR phyto data ready ####
phyto_raw <- read.csv("phyto.76-21_ZHR.csv", sep = ",")
...
ggplot(data = plot_all, aes(x = date, y = abundance)) + 
  ...
  ggtitle(label = "Abundance of Phyto taxa in ZHR lake")
dev.off()
setwd("~/ETHZ/Bachelor thesis/R/workplace2/data_new/")
...
names(taxa_noBiovol) <- c("id_CH", "genus", "species", "kremer_key", "abundance", "occurrences")
...
noBiovol$lake <- "ZHR"  # adding the lake id, to plot where the taxa missing a biolovume occur (see B files)

# exporting this data
setwd("~/ETHZ/Bachelor thesis/R/workplace2/no_biovol")
write.csv(noBiovol, "ZHR_phyto_datapoints_without_biovol.csv")  # all data points whose taxa don't have a biovolume in the database
write.csv(taxa_noBiovol, "ZHR_phyto_taxa_without_biovol.csv")  # same, resumed by taxa
setwd("~/ETHZ/Bachelor thesis/R/workplace2/data_new")
...
zoo_raw <- read.csv("largezoo.77-21_ZHR.csv")
...
ggplot(data = plot_allZ, aes(x = date, y = abundance)) + 
  ...
  scale_y_continuous(trans = "log10") + 
  labs(x = "Date", y = "Abundance [indv/L]", colour = "Biovolume?", shape = "Biovolume?") + 
  ggtitle(label = "Abundance of Zoo taxa in ZHR lake")
dev.off()
...
dim(taxa_noBiovolZ <- aggregate(list(noBiovolZ$abundance, noBiovolZ$occurrences), by = list(noBiovolZ$id_CH, noBiovolZ$genus, noBiovolZ$species), sum))  # There are 48 id_CH without a Biovolume
...
sum(taxa_noBiovolZ %>% filter(guild == "cyclopoid copepods") %>% select(occurrences))  # the cyclopod copepods represent 2008 occurrences out of 4463
sum(taxa_noBiovolZ %>% filter(guild == "calanoid copepods") %>% select(occurrences))  # the calanoid copepods represent 1384 occurrences out of 4463
sum(taxa_noBiovolZ %>% filter(guild %in% c("calanoid copepods", "herbivore cladocerans")) %>% select(occurrences))  # the herbivores represent 2123 occurrences out of 4463. And if we remove the cyclopoid copepods from that, it's 2123 out of 2455
...
noBiovolZ$lake <- "ZHR"  # adding the lake id, to plot where the taxa missing a biolovume occur (see B files)

# exporting the data without a biovolume
setwd("~/ETHZ/Bachelor thesis/R/workplace2/no_biovol")
write.csv(noBiovolZ, "ZHR_zoo_datapoints_without_biovol.csv")  # all data points whose taxa don't have a biovolume in the database
write.csv(taxa_noBiovolZ, "ZHR_zoo_taxa_without_biovol.csv")  # same, resumed by taxa
setwd("~/ETHZ/Bachelor thesis/R/workplace2/data_new")
...
figure_tempchem <- annotate_figure(figure_tempchem, top = text_grob("ZHR lake", face = "bold", size = 20))  # Adding a title to the figure
...
pairs_plot_logged <- pairs(logged_biovol,
                           ...
                           pch = 20,
                           cex = 0.5)
# Based on these plots, I think there might be a chance of slight linear relationship between temp and log(chrooco) and between temp and log(tot_phyt)

# Relationship between chrooco and temp:
fit_temp_chrooco <- lm(data = end_frame, chrooco ~ temperature)
summary(fit_temp_chrooco)  
cor(end_frame %>% select(chrooco, temperature) %>% na.omit() %>% select(chrooco) %>% summarize(chrooco = log(chrooco)), end_frame %>% select(chrooco, temperature) %>% na.omit() %>% select(temperature))
# not very significant pair-wise correlation of 0.320, p = 0.00823
par(mfrow = c(1,2))
plot(fit_temp_chrooco, which = c(1:2))
par(mfrow = c(1,1))
# the QQ-plot looks Horrible, and the TA-plot is very messed up --> shouldn't trust this model
...
# slightly significant pair-wise correlation of 0.561, but p = 7.871e-14
par(mfrow = c(1,2))
plot(fit_temp_totPhyto, which = c(1:2))
par(mfrow = c(1,1))
# On this one, the TA-plot looks and the GG-plot look ok (although the QQ-plot hasa slight curve)
...
par(mfrow = c(1,1))
# Both the TA-plot and the QQ-plot don't look ok --> shouldn't use a linear model for this either

# Making a pdf file out of the figure with all the plots, and saving it in the "graphs_lists" folder
setwd("~/ETHZ/Bachelor thesis/R/workplace2/graphs_lists")
pdf("ZHR_plot_temp_chem.pdf")
figure_tempchem
figure_all
figure_trends
