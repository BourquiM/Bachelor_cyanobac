...
phyto_raw <- read.csv("phyto.smallzoo.86-19_SEM.csv", sep = ",")  # loading the raw phytoplankton data
zoo_raw <- read.csv("largezoo.86-19_SEM.csv")  # loading the raw zooplankton data
...
lake <- "SEM"  # Defining the name of the lake. This will be used when saving data in external files, to identify them later on
...
rfProfile_knn 
rfProfile_bag 
rfProfile_median  # Temperature and PO4 seem to always be important predictors
...
# Unexpected: mtry = 1 was the best in all 3
...
print(impPlots <- ggarrange(knnPlot, bagPlot,medianPlot,
                            ncol = 1, nrow = 3, align = "hv"))
# Temperature and PO4 seem to always be important
