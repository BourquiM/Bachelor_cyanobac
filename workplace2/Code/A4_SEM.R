...
phyto_raw <- read.csv("phyto.smallzoo.86-19_SEM.csv", sep = ",")
...
# class(phyto_SEM$id_CH)  #it's an integer, so no need to change
...
{ 
  ...
  ggplot(data = plot_all, aes(x = date, y = abundance)) + 
    ...
    ggtitle(label = "Abundance of Phyto taxa in SEM lake")
  dev.off()
  setwd("~/ETHZ/Bachelor thesis/R/workplace2/data_new/")
}
...
# There are 5 taxa without a biovolume
...
noBiovol$lake <- "SEM"  # adding the lake id, to plot where the taxa missing a biolovume occur (see B files)

# exporting this data
setwd("~/ETHZ/Bachelor thesis/R/workplace2/no_biovol")
write.csv(noBiovol, "SEM_phyto_datapoints_without_biovol.csv")  # all data points whose taxa don't have a biovolume in the database
write.csv(taxa_noBiovol, "SEM_phyto_taxa_without_biovol.csv")  # same, resumed by taxa
setwd("~/ETHZ/Bachelor thesis/R/workplace2/data_new")
...
zoo_raw <- read.csv("largezoo.86-19_SEM.csv")
...
ggplot(data = plot_allZ, aes(x = date, y = abundance)) + 
  ...
  scale_y_continuous(trans = "log10") + 
  labs(x = "Date", y = "Abundance [indv/L]", colour = "Biovolume?", shape = "Biovolume?") + 
  ggtitle(label = "Abundance of Zoo taxa in SEM lake")
dev.off()
...
noBiovolZ$lake <- "SEM"  # adding the lake id, to plot where the taxa missing a biolovume occur (see B files)

# exporting the data without a biovolume
setwd("~/ETHZ/Bachelor thesis/R/workplace2/no_biovol")
write.csv(noBiovolZ, "SEM_zoo_datapoints_without_biovol.csv")  # all data points whose taxa don't have a biovolume in the database
write.csv(taxa_noBiovolZ, "SEM_zoo_taxa_without_biovol.csv")  # same, resumed by taxa
setwd("~/ETHZ/Bachelor thesis/R/workplace2/data_new")
...
temp_chem <- read.csv("temp.chem.85-20_SEM.csv")
...
figure_tempchem <- annotate_figure(figure_tempchem, top = text_grob("SEM lake", face = "bold", size = 20))  # Adding a title to the figure
...
pairs_plot_logged <- pairs(logged_biovol,
                           ...
                           pch = 20,
                           cex = 0.5)
# Based on these plots, I think there might be a linear relationship between parameters no3 and po4

# Relationship between no3 and po4:
fit_no3_po4 <- lm(data = end_frame, PO4_P ~ NO3_N)
summary(fit_no3_po4)  
cor(end_frame %>% select(NO3_N, PO4_P) %>% na.omit() %>% select(NO3_N), end_frame %>% select(NO3_N, PO4_P) %>% na.omit() %>% select(PO4_P))
# significant pair-wise correlation of 0.687, p < 2e-16
par(mfrow = c(1,2))
plot(fit_no3_po4, which = c(1:2))
par(mfrow = c(1,1))
# the QQ-plot hat too much of a bent upper tail, and the TA-plot is very messed up --> better not use a linear regression to explain this relationship
...
par(mfrow = c(1,1))
# On this one, the TA-plot looks and the GG-plot look very messed up (TA has a triangular shape, and the QQ is too bent)
