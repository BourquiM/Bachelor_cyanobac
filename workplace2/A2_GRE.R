...
# 1) Getting the GRE phyto data ready ####
phyto_raw <- read.csv("phyto.smallzoo.85-20_GRE.csv", sep = ",", encoding = "URF-8")
...
# Taking care of the id_CH
class(phyto_raw$id_CH)  #it's a chr, means I'll have to change that
...
phyto_raw <- phyto_raw %>% mutate(id_CH = replace(id_CH, id_CH == "Scenedesmus linearis", 3025)) # replaces the "Scenedesmus linearis" by its real id_CH. I chose to keep this taxon only
...
dim(noBiovol <- subset(phyto_selec, is.na(volume_um.3)))  #there are 462 rows without a volume_um.3 biovolume...
# Now combined with the kremer biovolume:
dim(noBiovol <- subset(noBiovol, is.na(mdn_nu.biovol))) # 456 entries left
...
ggplot(data = plot_all, aes(x = date, y = abundance)) + 
  ...
  ggtitle(label = "Abundance of Phyto taxa in GRE lake")
dev.off()
setwd("~/ETHZ/Bachelor thesis/R/workplace2/data_new/")


# Taking a look at the taxa without a biovolume
noBiovol <- noBiovol %>% arrange(id_CH)
noBiovol$occurrences <- 1 # this column will be used to count the amount of times the taxa were observed
...
noBiovol$lake <- "GRE"  # adding the lake id, to plot where the taxa missing a biolovume occur (see B files)

# exporting this data
setwd("~/ETHZ/Bachelor thesis/R/workplace2/no_biovol")
write.csv(noBiovol, "GRE_phyto_datapoints_without_biovol.csv")  # all data points whose taxa don't have a biovolume in the database
write.csv(taxa_noBiovol, "GRE_phyto_taxa_without_biovol.csv")  # same, resumed by taxa
setwd("~/ETHZ/Bachelor thesis/R/workplace2/data_new")



# 3) Now the phyto data with a Biovolume, preparing it for the final data table ####
# These lines of code add a biovolume to the taxa that don't have one.
# TODO It is ready. Just need to replace the NA by the found biovolume
...
dim(noBiovolZ <- subset(zoo_selec, is.na(volume_um.3)))  #there are 601 rows without a volume_um.3 BiovolZume -> that's 7.73% of the data
...
ggplot(data = plot_allZ, aes(x = date, y = abundance)) + 
  ...
  scale_y_continuous(trans = "log10") + 
  labs(x = "Date", y = "Abundance [indv/L]", colour = "Biovolume?", shape = "Biovolume?") + 
  ggtitle(label = "Abundance of Zoo taxa in GRE lake")
dev.off()
...
dim(taxa_noBiovolZ <- aggregate(list(noBiovolZ$abundance, noBiovolZ$occurrences), by = list(noBiovolZ$id_CH, noBiovolZ$genus, noBiovolZ$species), sum))  # There are 4 id_CH without a Biovolume
...
sum(taxa_noBiovolZ %>% filter(guild == "cyclopoid copepods") %>% select(occurrences))  # the cyclopod copepods represent 400 occurrences out of 601
sum(taxa_noBiovolZ %>% filter(guild == "calanoid copepods") %>% select(occurrences))  # the calanoid copepods represent 200 occurrences
sum(taxa_noBiovolZ %>% filter(guild %in% c("calanoid copepods", "herbivore cladocerans")) %>% select(occurrences))  # the herbivores represent 201 occurrences out of 601 And if we remove the cyclopoid copepods from that, it's 201 out of 201
...
noBiovolZ$lake <- "GRE"  # adding the lake id, to plot where the taxa missing a biolovume occur (see B files)

# exporting the data without a biovolume
setwd("~/ETHZ/Bachelor thesis/R/workplace2/no_biovol")
write.csv(noBiovolZ, "GRE_zoo_datapoints_without_biovol.csv")  # all data points whose taxa don't have a biovolume in the database
write.csv(taxa_noBiovolZ, "GRE_zoo_taxa_without_biovol.csv")  # same, resumed by taxa
setwd("~/ETHZ/Bachelor thesis/R/workplace2/data_new")
...
figure_tempchem <- annotate_figure(figure_tempchem, top = text_grob("GRE lake", face = "bold", size = 20))  # Adding a title to the figure
...
ratios <- ggplot(data = end_frame) +
  ...
  scale_y_continuous(name = "Ratio Nostocales", lim = c(0, 1), labels = scales::percent, sec.axis = sec_axis(trans =~., name = "Ratio Chroococcales", labels = scales::percent)) + 
...
pairs_plot_logged <- pairs(logged_biovol,
                           ...
                           pch = 20,
                           cex = 0.5)
# Based on these plots, I think there might be a chance of linear relationship between nh4 and no3 and between nh4 and po4 

# Relationship between nh4 and no3:
fit_nh4_no3 <- lm(data = end_frame, NH4_N ~ NO3_N)
summary(fit_nh4_no3)  
cor(end_frame %>% select(NO3_N, NH4_N) %>% na.omit() %>% select(NH4_N), end_frame %>% select(NO3_N, NH4_N) %>% na.omit() %>% select(NO3_N))
# highly significant pair-wise correlation of -0.791, p < 2e-16
par(mfrow = c(1,2))
plot(fit_nh4_no3, which = c(1:2))
par(mfrow = c(1,1))
# the QQ-plot looks somewhat ok (also it is slightly bent), but the TA-plot is very messed up (triangle shape)


# Relationship between nh4 and po4
fit_nh4_po4 <- lm(data = end_frame, NH4_N ~ PO4_P)
summary(fit_nh4_po4)
cor(end_frame %>% select(NH4_N, PO4_P) %>% na.omit() %>% select(NH4_N), end_frame %>% select(NH4_N, PO4_P) %>% na.omit() %>% select(PO4_P))
# highly significant pair-wise correlation of 0.697, p < 2e-16
par(mfrow = c(1,2))
plot(fit_nh4_po4, which = c(1:2))
par(mfrow = c(1,1))
# The QQ-Plot looks good, but the TA-Plot has a clear diamond/lozenge shape
...
par(mfrow = c(1,1))
# Both the TA-plot and the QQ-plot look acceptable --> we should be able to trust this model

# Making a pdf file out of the figure with all the plots, and saving it in the "graphs_lists" folder
setwd("~/ETHZ/Bachelor thesis/R/workplace2/graphs_lists")
pdf("GRE_plot_temp_chem.pdf")
figure_tempchem
figure_all
figure_trends
